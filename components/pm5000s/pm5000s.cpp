...
void PM5000SSensor::update() {
  ...
  auto get_u32 = [](uint8_t *buf, int idx) {
    return (uint32_t)buf[idx] << 24 | (uint32_t)buf[idx + 1] << 16 | (uint32_t)buf[idx + 2] << 8 | (uint32_t)buf[idx + 3];
  };
  ...
  ESP_LOGI("pm5000s", "Particles [pcs/L]: >0.3µm=%u >0.5µm=%u >1.0µm=%u >2.5µm=%u >5.0µm=%u >10µm=%u",
